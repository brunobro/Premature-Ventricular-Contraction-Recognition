...
São considerados dados ruidosos, onde a média da SNR é
treinamento: -0.9080
teste: -0.5661
...
def print_measures(TP, TN, FP, FN):   
    ...
    print('[', round(Acc, 4), ',', round(F_P, 4), ',', round(F_N, 4), ',', round(Se, 4), ',', round(Sp, 4), ',', round(Pr_P, 4), ',', round(Pr_N, 4), ']')'''
    print('[', round(Acc, 4), ']')
...
'''
features0_train, targets_new = amostras_artificiais(features0_train, targets_train)
features1_train              = amostras_artificiais(features1_train, targets_train, False)

targets_train = targets_new
del targets_new
'''
...
'''
features0_test = tools.rectify(np.loadtxt(config.DIR_FILES + '2componentes/' + config.FILE_FEATURES_TEST + '_' + config.DS_TEST  + '_' + len_cycle_tra + '_noise_0.txt'))
features1_test = tools.rectify(np.loadtxt(config.DIR_FILES + '2componentes/' + config.FILE_FEATURES_TEST + '_' + config.DS_TEST  + '_' + len_cycle_tra + '_noise_1.txt'))
targets_test   = np.loadtxt(config.DIR_FILES + config.FILE_TARGET_TEST + '_' + config.DS_TEST  + '_' + len_cycle_tra + '.txt')

'''
features0_test, targets_new = amostras_artificiais(features0_test, targets_test)
features1_test              = amostras_artificiais(features1_test, targets_test, False)

targets_test = targets_new
del targets_new

print('Batimentos:')
print('\t Treinamento: ', len(features1_train))
print('\t Teste: ', len(features1_test))
'''
...
for m in M:
    ...
    print(' P+:  ', round(m[5], 4))
    print(' P-:  ', round(m[6], 4))
    print('[', round(m[0], 4), ',', round(m[1], 4), ',', round(m[2], 4), ',', round(m[3], 4), ',', round(m[4], 4), ',', round(m[5], 4), ',', round(m[6], 4), ']')'''
    print('[', round(m[0], 4), ']')
    i += 1
