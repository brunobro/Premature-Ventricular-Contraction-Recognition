...
'''
features0_train = tools.rectify(np.loadtxt(config.DIR_FILES + '4componentes/' + config.FILE_FEATURES_TRAINING + '_' + config.DS_TRAINING + '_' + len_cycle_tra + '_0.txt'))
features1_train = tools.rectify(np.loadtxt(config.DIR_FILES + '4componentes/' + config.FILE_FEATURES_TRAINING + '_' + config.DS_TRAINING + '_' + len_cycle_tra + '_1.txt'))
features2_train = tools.rectify(np.loadtxt(config.DIR_FILES + '4componentes/' + config.FILE_FEATURES_TRAINING + '_' + config.DS_TRAINING + '_' + len_cycle_tra + '_2.txt'))
features3_train = tools.rectify(np.loadtxt(config.DIR_FILES + '4componentes/' + config.FILE_FEATURES_TRAINING + '_' + config.DS_TRAINING + '_' + len_cycle_tra + '_3.txt'))
...
features3_train = LinearDiscriminantAnalysis().fit_transform(features3_train, targets_train)

'''
Carrega os dados de teste
'''
features0_test = tools.rectify(np.loadtxt(config.DIR_FILES + '4componentes/' + config.FILE_FEATURES_TEST + '_' + config.DS_TEST + '_' + len_cycle_tes + '_0.txt'))
features1_test = tools.rectify(np.loadtxt(config.DIR_FILES + '4componentes/' + config.FILE_FEATURES_TEST + '_' + config.DS_TEST + '_' + len_cycle_tes + '_1.txt'))
features2_test = tools.rectify(np.loadtxt(config.DIR_FILES + '4componentes/' + config.FILE_FEATURES_TEST + '_' + config.DS_TEST + '_' + len_cycle_tes + '_2.txt'))
features3_test = tools.rectify(np.loadtxt(config.DIR_FILES + '4componentes/' + config.FILE_FEATURES_TEST + '_' + config.DS_TEST + '_' + len_cycle_tes + '_3.txt'))
...
features3_test = LinearDiscriminantAnalysis().fit_transform(features3_test, targets_test)
...
if SAVE_MACHINE:
    ...
    features3_train = features3_train[:,:LEN_FEATURES]
...
if SAVE_MACHINE:
    ...
    features3_test = features3_test[:,:LEN_FEATURES]
...
if SAVE_MACHINE:
    if M0[0] >= 0.8:
        joblib.dump(clf0, config.DIR_FILES_MACHINES + 'clf_III0.sav')
        
    if M1[0] >= 0.8:
        joblib.dump(clf1, config.DIR_FILES_MACHINES + 'clf_III1.sav')
        
    if M2[0] >= 0.8:
        joblib.dump(clf2, config.DIR_FILES_MACHINES + 'clf_III2.sav')
        
    if M3[0] >= 0.8:
        joblib.dump(clf3, config.DIR_FILES_MACHINES + 'clf_III3.sav')
...
for k in range(0, targets_test.shape[0]):
    ...
    predicted_proba_clf3 = clf3.predict_proba(features3_test[k].reshape(-1, 1).T)
    
    #Calcula a media ponderada das probabilidades M_proba[0] classe negativa, M_proba[1] classe positiva
    M_proba = predicted_proba_clf0[0] + predicted_proba_clf1[0] + predicted_proba_clf2[0] + predicted_proba_clf3[0]
...
for k in range(0, targets_test.shape[0]):
    ...
    ps = np.array([predicted_clf0, predicted_clf1, predicted_clf2, predicted_clf3])
...
#Resultados
Acc = [M0[0], M1[0], M2[0], M3[0]]
F_P = [M0[1], M1[1], M2[1], M3[1]]
F_N = [M0[2], M1[2], M2[2], M3[2]]
Se  = [M0[3], M1[3], M2[3], M3[3]]
Sp  = [M0[4], M1[4], M2[4], M3[4]]
Pr_P = [M0[5], M1[5], M2[5], M3[5]]
Pr_N = [M0[6], M1[6], M2[6], M3[6]]
...
clf3_results = measures_arr[3,]

measures_clfs = np.array([clf0_results, clf1_results, clf2_results, clf3_results])
...
for k in range(0, targets_test.shape[0]):
    ...
    predicted_proba_clf3 = v[3] * clf3.predict_proba(features3_test[k].reshape(-1, 1).T)
...
print('\nDados AHP')
print('Clf0, Clf1, Clf2, Clf3: ', np.round(v, 4))
