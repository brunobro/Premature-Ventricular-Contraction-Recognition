...
'''
clf0 = AdaBoostClassifier(base_estimator=GaussianNB(), n_estimators=4, random_state=0)
clf1 = AdaBoostClassifier(base_estimator=GaussianNB(), n_estimators=4, random_state=0)
clf2 = AdaBoostClassifier(base_estimator=GaussianNB(), n_estimators=4, random_state=0)
clf3 = AdaBoostClassifier(base_estimator=GaussianNB(), n_estimators=4, random_state=0)
...
for i in range(0, features0_train.shape[0]):
    features_train.append(np.concatenate((features0_train[i,:], features1_train[i,:], features2_train[i,:], features3_train[i,:])))

features_train = np.array(features_train)

del features0_train, features1_train, features2_train, features3_train

features_test = []
for i in range(0, features0_test.shape[0]):
    features_test.append(np.concatenate((features0_test[i,:], features1_test[i,:], features2_test[i,:], features3_test[i,:])))

features_test = np.array(features_test)

del features0_test, features1_test, features2_test, features3_test
...
Treinamento dos Classificadores
'''
clf = AdaBoostClassifier(base_estimator=GaussianNB(), n_estimators=4, random_state=0)
