...
for i in range(0, int((len(v_trai) + len(v_test))/2)):
    ...
    '''
    print('\nLendo Dados Treinamento')
    for record in v_trai: 
        print('\tRegistro', record)
        
        CONFIG_FILE   = config.FILE_FEATURES_TEST
        CONFIG_TARGET = config.FILE_TARGET_TEST
        CONFIG_DS     = config.DS_TEST
        if record in eval(config.DS_TRAINING):
            CONFIG_FILE   = config.FILE_FEATURES_TRAINING
            CONFIG_TARGET = config.FILE_TARGET_TRAINING
            CONFIG_DS     = config.DS_TRAINING
            
        rec0 = config.DIR_FILES + '2componentes/' + CONFIG_FILE + '_' + CONFIG_DS + '_' + len_cycle_tra + '_' + str(record) + '_0.txt'
        ...
        if os.path.exists(rec0):
            ...
            data1 = np.loadtxt(config.DIR_FILES + '2componentes/' + CONFIG_FILE + '_' + CONFIG_DS + '_' + len_cycle_tra + '_' + str(record) + '_1.txt')
            F1 = tools.rectify(change_shape(data1))
            
            L_TRAI = np.loadtxt(config.DIR_FILES_RECORDS + CONFIG_TARGET + '_' + CONFIG_DS + '_' + len_cycle_tra + '_' + str(record) + '.txt')
        ...
        print('\tInstâncias: ', len(LABELS_TRAI))
    '''
    ...
    for record in v_test:    
        ...
        if os.path.exists(rec0):
            ...
            L_TEST = np.loadtxt(config.DIR_FILES_RECORDS + CONFIG_TARGET + '_' + CONFIG_DS + '_' + len_cycle_tra + '_' + str(record) + '.txt')
        ...
        print('\tInstâncias: ', len(LABELS_TEST))
    ...
    FEATURES0_TRAI  = LinearDiscriminantAnalysis().fit_transform(FEATURES0_TRAI, LABELS_TRAI)
    FEATURES1_TRAI  = LinearDiscriminantAnalysis().fit_transform(FEATURES1_TRAI, LABELS_TRAI)
    ...
    r = '\n######## Dobra ' + str(fold) + ' ########'         
    r += '\n Registros:'
    r += '\n  Treino: ' + str(LABELS_TRAI.size) + ' = ' + str(len(np.where(LABELS_TRAI == 1.0)[0])) + ' CPV e ' + str(len(np.where(LABELS_TRAI == 0.0)[0])) + ' Normal \n' + str(v_trai).replace('\n','')
    r += '\n  Teste: ' + str(LABELS_TEST.size) + ' = ' + str(len(np.where(LABELS_TEST == 1.0)[0])) + ' CPV e ' + str(len(np.where(LABELS_TEST == 0.0)[0])) + ' Normal \n' + str(v_test).replace('\n','')
    r += '\n Resultados:'
    r += R
    print(r.replace('.0000',''))
    ...
    '''
    Altera os registros, trocando um que estava no conjunto de teste, por outro do conjunto de treinamento
    '''
