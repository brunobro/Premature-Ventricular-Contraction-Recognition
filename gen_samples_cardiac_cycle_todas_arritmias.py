...
def __getCardiacCycle(RECORD, DATABASE, ANN_TYPE):
    ...
    for l in range(0, len(ann.sample)):
        ...
        if ANN_TYPE == 'V':
            if sum(cycle_seg) != 0 and len(cycle_seg) == len_cycle and ann_type == 'V':
...
def __createSamples(RECORDS, LABEL_RECORDS, DATABASE, FILE_SAVE, TEXT):
    ...
    np.savetxt(config.DIR_FILES + FILE_SAVE + '_' + LABEL_RECORDS + '_' + str(config.LEN_CARDIAC_CYCLE(DATABASE)) + '_todas_arritmias.txt', beats, delimiter=' ', fmt='%1.4f')
    np.savetxt(config.DIR_FILES + 'target_' + FILE_SAVE + '_' + LABEL_RECORDS + '_' + str(config.LEN_CARDIAC_CYCLE(DATABASE)) + '_todas_arritmias.txt', target, delimiter=' ', fmt='%d')
    
    #banco de dados balanceado
    np.savetxt(config.DIR_FILES + FILE_SAVE + '_' + LABEL_RECORDS + '_' + config.FILE_SAMPLES_BALANCED + '_' + str(config.LEN_CARDIAC_CYCLE(DATABASE)) + '_todas_arritmias.txt', beats_bal, delimiter=' ', fmt='%1.4f')
    np.savetxt(config.DIR_FILES + 'target_' + FILE_SAVE + '_' + LABEL_RECORDS + '_' + config.FILE_SAMPLES_BALANCED + '_' + str(config.LEN_CARDIAC_CYCLE(DATABASE)) + '_todas_arritmias.txt', target_bal, delimiter=' ', fmt='%d')
    ...
    f = open(config.DIR_FILES + 'info_' + FILE_SAVE + '_' + LABEL_RECORDS + '.txt', 'w')
    f.truncate()
    f.write(TEXT)
    f.close()
