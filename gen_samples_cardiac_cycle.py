...
def __createSamples(RECORDS, LABEL_RECORDS, DATABASE, FILE_SAVE, TEXT):
    ...
    np.savetxt(config.DIR_FILES + FILE_SAVE + '_' + LABEL_RECORDS + '_' + str(config.LEN_CARDIAC_CYCLE(DATABASE)) + '.txt', beats, delimiter=' ', fmt='%1.4f')
    np.savetxt(config.DIR_FILES + 'target_' + FILE_SAVE + '_' + LABEL_RECORDS + '_' + str(config.LEN_CARDIAC_CYCLE(DATABASE)) + '.txt', target, delimiter=' ', fmt='%d')
    
    #banco de dados balanceado
    np.savetxt(config.DIR_FILES + FILE_SAVE + '_' + LABEL_RECORDS + '_' + config.FILE_SAMPLES_BALANCED + '_' + str(config.LEN_CARDIAC_CYCLE(DATABASE)) + '.txt', beats_bal, delimiter=' ', fmt='%1.4f')
    np.savetxt(config.DIR_FILES + 'target_' + FILE_SAVE + '_' + LABEL_RECORDS + '_' + config.FILE_SAMPLES_BALANCED + '_' + str(config.LEN_CARDIAC_CYCLE(DATABASE)) + '.txt', target_bal, delimiter=' ', fmt='%d')
