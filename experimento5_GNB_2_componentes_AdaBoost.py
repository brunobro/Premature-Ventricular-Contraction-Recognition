...
'''
clf0 = AdaBoostClassifier(base_estimator=GaussianNB(), n_estimators=2, random_state=0)
clf1 = AdaBoostClassifier(base_estimator=GaussianNB(), n_estimators=2, random_state=0)
...
print('Performances considerando duas máquinas distintas (duas componentes)')
...
for i in range(0, features0_train.shape[0]):
    features_train.append(np.concatenate((features0_train[i,:], features1_train[i,:])))
...
for i in range(0, features0_test.shape[0]):
    features_test.append(np.concatenate((features0_test[i,:], features1_test[i,:])))
...
Treinamento dos Classificadores
'''
clf = AdaBoostClassifier(base_estimator=GaussianNB(), n_estimators=2, random_state=0)
